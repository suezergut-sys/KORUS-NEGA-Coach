...
При сохранении анализа `training_sessions.user_id` заполняется идентификатором текущего участника, а `participant_role_name` — именем выбранной роли. `/account` объединяет его сессии, кейсы и результаты `evaluations` для построения истории, а `/rating` выполняет агрегацию по всем обычным пользователям. Победой считается только результат анализа с `outcome.winner = "user"`; ничья и победа оппонента увеличивают число поединков, но не число побед. Средний балл считается по оценённым сессиям среди десяти последних поединков пользователя.
...
| `training_sessions` | Метаданные проведённых тренировок, выбранная роль и ссылка на участника |
