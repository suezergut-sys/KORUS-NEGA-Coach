...
- после быстрой канонизации или утверждения варианта запускается фоновый медиаконвейер: четыре общих комикс-кадра через `gpt-image-2`, отдельные подписи и аудиорассказ через Speech API для каждой роли, загрузка в приватный Supabase Storage и публикация только готового пакета;
...
- имена объектов Supabase Storage нормализуются в безопасный ASCII-ключ, поэтому DOCX/PDF с русскими названиями загружаются без `Invalid key`; исходное имя файла сохраняется отдельно в базе и показывается пользователю;
