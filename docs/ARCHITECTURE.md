...
Статус: реализация v0.7. Голосовой прототип работает; источники Тарасова и Гарвардского метода импортированы в приватный Supabase Storage и pgvector. Все 34 атома Тарасова временно подтверждены владельцем как `tarasov-v1`; 37 атомов Гарвардского метода ожидают экспертной проверки как `harvard-v0-candidate`. В настройках тренировки, файловом анализе и панели методолога доступен выбор методологии. Реализованы библиотека кейсов, быстрая загрузка, AI-конструктор, корпоративная регистрация через Supabase Auth, персональная статистика и общий рейтинг участников.
