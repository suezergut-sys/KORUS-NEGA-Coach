...
Статус: реализация v0.3. Голосовой прототип работает; книга импортирована в приватный Supabase Storage и pgvector. Извлечены первые 34 кандидата методических атомов. Пост-анализ сохраняет стенограмму и показывает только подтверждённые дословные цитаты. До экспертной проверки оценка маркируется как предварительная (`tarasov-v0-candidate`).
