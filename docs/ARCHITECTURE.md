...
Автоматическое производство включено для обоих путей публикации: быстрой загрузки готового кейса и утверждения варианта из конструктора. После ответа пользователю `after()` продолжает фоновую задачу в пределах `maxDuration`: создаёт четыре общих изображения, затем отдельную четырёхсценную аудиоверсию и подписи для каждой из 2–4 ролей. Статусы `pending/processing/ready/failed` хранятся в `case_media_jobs`, панели — в `case_comic_panels`, бинарные WEBP/MP3 — в закрытом bucket `case-comics`. Клиент получает только часовые signed URLs.
