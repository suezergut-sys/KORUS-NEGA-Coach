...
Канонический кейс хранит ситуацию, центральный конфликт, две персонифицированные стороны, публичные и скрытые параметры ролей, интересы, ограничения, рычаги, ставки, стартовую позицию, причину сложности, критерии оценки и ссылки на методические атомы. Для каждой стороны `name` обязательно содержит минимум имя и фамилию; должность и организационная роль записываются отдельно в `position`. Это требование одновременно присутствует в промпте, строгой JSON Schema и проверке перед публикацией. Безымянный вариант или обозначение вида «руководитель проекта» вместо ФИО не считается каноническим и не может попасть в `negotiation_cases`. Скрытые мотивы используются серверным Realtime-промптом и не показываются участнику как готовый ответ.
