...
В прототипе реализован проигрыватель комикс-пролога и эталонная история из четырёх кадров для кейса «Сорванный срок проекта». `getCaseComic` связывает версию кейса с изображениями, заголовками и отдельным текстом каждой сцены. `/api/cases/narration` принимает `panelIndex`, озвучивает только активную сцену выбранным голосом, а клиент после окончания аудио автоматически переключает кадр. Полное каноническое описание остаётся доступно из того же окна.
...
5. Готовая история хранится по версии канонического кейса и показывается перед стартом как перелистываемая лента. Озвучивание синхронизируется с кадрами по временным меткам, но пользователь всегда может пропустить пролог.
