...
as $$
  with profile_stats as (
    select
      ...
      count(sessions.id) filter (
        where sessions.case_code <> '1c-dismissal'
          and evaluations.result->'outcome'->>'winner' = 'user'
      )::bigint as wins,
      case
        when count(sessions.id) filter (where sessions.case_code <> '1c-dismissal') = 0 then 0
        else round(
          100.0 * count(sessions.id) filter (
            where sessions.case_code <> '1c-dismissal'
              and evaluations.result->'outcome'->>'winner' = 'user'
          ) / count(sessions.id) filter (where sessions.case_code <> '1c-dismissal')
        )::integer
      end as win_rate,
  ...
  left join lateral (
    select jsonb_agg(
      jsonb_build_object(
        'id', case when recent.visibility = 'private' and recent.owner_user_id is distinct from p_requesting_user_id then null else recent.case_id end,
        'name', case when recent.visibility = 'private' and recent.owner_user_id is distinct from p_requesting_user_id then 'Приватный кейс пользователя' else recent.case_name end,
        'playable', recent.is_default or recent.visibility = 'public' or recent.owner_user_id = p_requesting_user_id,
    ...
    from (
      ...
      from (
        select distinct on (coalesce(sessions.case_id::text, sessions.case_code))
          ...
          coalesce(cases.title, case when sessions.case_code = 'missed-project-deadline' then 'Сорванный срок проекта' else sessions.case_code end) as case_name
...
comment on function public.get_rating_page(uuid, integer, integer, text, boolean) is
  'Общий рейтинг: кейс 1С Увольнение учитывается в тренировках и среднем балле, но не в победах и проценте побед.';
