...
const DELIVERY_BY_TONE: Record<OpponentEmotionTone, string> = {
  calm: "Говори спокойно, ровно и уверенно. Сохраняй деловой темп и естественный ритм.",
  guarded: "Говори заметно сдержаннее и суше предыдущей спокойной реплики. Перед ответом на мгновение промолчи, чуть замедли темп, проверяй конкретику и не спеши доверять обещаниям.",
  ...
  irritated: "Говори явно холоднее и жёстче предыдущей реплики: короткими фразами, с плотной артикуляцией и напряжёнными смысловыми остановками. Раздражение должно быть слышно, но не повышай голос и не груби.",
...
export function buildOpponentEmotionInstructions(
  ...
  const reaction = triggers.length
    ...
    : "В последнем ходе нет отдельного сильного триггера. Сохраняй накопленное состояние, не возвращайся автоматически к нейтральной подаче.";
  const calmBoundary = addressForm === "informal" ? "Дай мне закончить мысль" : "Дайте мне закончить мысль";
  const hardBoundaries = addressForm === "informal" ? "«Не перебивай меня» или «Дай договорить»" : "«Не перебивайте меня» или «Дайте договорить»";
  const interruptionDirection = triggers.includes("interruption")
    ? state.tone === "guarded"
      ? `Пользователь перебил тебя. На мгновение промолчи, затем спокойно, но явно обозначь границу фразой вроде «${calmBoundary}» и только после этого отвечай по существу. Смена интонации должна быть различима на слух уже в этой реплике.`
      : `Пользователь снова или особенно резко перебил тебя. Сразу жёстко обозначь границу фразой вроде ${hardBoundaries}; произнеси её холодно и отрывисто, затем отвечай по существу. Реакция должна быть безошибочно заметна на слух.`
    : "";

  return `
# Эмоциональная режиссура этой реплики
Это самая новая и единственная актуальная эмоциональная режиссура. Она заменяет все предыдущие блоки эмоциональной режиссуры в истории разговора.
...
Вырази состояние естественно через интонацию, темп, ритм, краткость и выбор слов. Учитывай также реально услышанную интонацию пользователя и весь предыдущий разговор. Выводи только прямую речь персонажа. Все указания по исполнению выполни молча: не озвучивай названия действий и не добавляй сценические ремарки отдельными словами. Не называй служебные эмоции, состояние, триггеры или числовые значения вслух. Не переигрывай; явная словесная граница после перебивания обязательна и не считается переигрыванием. Сохраняй роль, цели, ограничения и правила переговоров из инструкций сессии.
`.trim();
}
