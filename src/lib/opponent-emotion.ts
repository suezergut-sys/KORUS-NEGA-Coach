...
export function buildOpponentEmotionInstructions(
  ...
  const interruptionDirection = triggers.includes("interruption")
    ? state.tone === "guarded"
      ? "Пользователь перебил тебя. После короткой заметной паузы спокойно, но явно обозначь границу фразой вроде «Дайте мне закончить мысль» и только затем отвечай по существу. Смена интонации должна быть различима на слух уже в этой реплике."
