...
export function buildOpponentEmotionInstructions(
  ...
  const interruptionDirection = triggers.includes("interruption")
    ? state.tone === "guarded"
      ? "Пользователь перебил тебя. На мгновение промолчи, затем спокойно, но явно обозначь границу фразой вроде «Дайте мне закончить мысль» и только после этого отвечай по существу. Смена интонации должна быть различима на слух уже в этой реплике."
      : "Пользователь снова или особенно резко перебил тебя. Сразу жёстко обозначь границу фразой вроде «Не перебивайте меня» или «Дайте договорить»; произнеси её холодно и отрывисто, затем отвечай по существу. Реакция должна быть безошибочно заметна на слух."
    : "";
