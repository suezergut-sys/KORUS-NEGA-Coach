...
export async function generateCaseVariants(input: { title: string; notes: string; materials: Material[] }) {
  ...
  const response = await getOpenAI().responses.create({
    ...
    instructions: `
Ты проектируешь русскоязычные учебные кейсы управленческих поединков по проверенной базе методологии Владимира Тарасова.
...
Канонический вариант должен содержать описание ситуации, центральный конфликт, роли, цели, интересы, ограничения, скрытые мотивы, рычаги, ставки, стартовую реплику/позицию и критерии оценки.
