...
9. Дополнительные поля заполняй только фактами из материалов. Сценарные условия сохраняй отдельно в scenarioConditions: это обязательные события, последовательность, минимальное число действий или иные правила развития разговора. Если подробная задача роли, стартовая реплика, типовые возражения, рекомендуемые или запрещённые реплики, сценарные условия, условия решения, границы полномочий, опасные зоны, успешный итог, следующие шаги или методические пояснения не заданы — возвращай пустую строку или пустой массив.

Канонический вариант должен содержать описание ситуации, центральный конфликт, две стороны с полными личными именами и отдельными должностями, цели, интересы, ограничения, скрытые мотивы, рычаги, ставки, стартовую реплику/позицию и критерии оценки. Сохраняй конкретные реплики и запреты в предназначенных для них структурированных полях, не растворяй их только в общем описании.
...
Сохрани структурированные дополнительные поля: roleBrief, openingLine, typicalObjections, recommendedPhrases, forbiddenPhrases у ролей, а также scenarioConditions, decisionTerms, authorityLimits, riskZones, successOutcome, expectedNextSteps и methodologyNotes у кейса. Меняй их только по прямым корректировкам пользователя.
    `.trim(),
