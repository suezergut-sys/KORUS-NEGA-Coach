...
9. Дополнительные поля заполняй только фактами из материалов. Если подробная задача роли, стартовая реплика, типовые возражения, рекомендуемые или запрещённые реплики, условия решения, границы полномочий, опасные зоны, успешный итог, следующие шаги или методические пояснения не заданы — возвращай пустую строку или пустой массив.
...
Сохрани структурированные дополнительные поля: roleBrief, openingLine, typicalObjections, recommendedPhrases, forbiddenPhrases у ролей, а также decisionTerms, authorityLimits, riskZones, successOutcome, expectedNextSteps и methodologyNotes у кейса. Меняй их только по прямым корректировкам пользователя.
    `.trim(),
