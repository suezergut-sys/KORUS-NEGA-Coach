...
export async function POST(request: Request) {
  ...
  try {
    ...
    const visibility = parseCaseVisibility(form.get("visibility"));
    if (!visibility) return Response.json({ error: "Выберите, кто сможет отрабатывать кейс." }, { status: 400 });
    const workspace = await createOrUpdateWorkspace({ title, notes: "Быстрая загрузка одного файла", ownerUserId: session.userId });
    workspaceId = workspace.id;
    await addWorkspaceFiles(workspace.id, [file], session.userId);
    ...
    const saved = await saveGeneratedVariants(workspace.id, variants, session.userId);
    ...
    if (workspaceId && !published) await discardWorkspace(workspaceId, session.userId);
