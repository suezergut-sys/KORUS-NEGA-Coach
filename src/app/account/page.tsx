        ...
        <section className="duel-history-card neon-panel">
          ...
          <div className="duel-history-wrap">
            <table className="duel-history-table">
              ...
              <tbody>{dashboard.history.map((duel) => <tr key={duel.id}><td>{historyDate(duel.endedAt)}</td><td><Link href={`/account/sessions/${duel.id}`}><strong>{duel.caseName}</strong></Link>{!duel.ranked && <small>нерейтинговый</small>}</td><td>{duel.participantRole}</td><td>{duel.result === null ? <span aria-label="Победитель для этого кейса не определяется">—</span> : <Link href={`/account/sessions/${duel.id}`} className={`duel-result ${duel.result === "Победа" ? "win" : duel.result === "Поражение" ? "loss" : "draw"}`} title="Стенограмма и отчёт" aria-label={`${duel.status === "analysis_failed" ? "Нужен повтор" : duel.result}. Стенограмма и отчёт`}>{duel.status === "analysis_failed" ? "Нужен повтор" : duel.result}</Link>}</td><td><b className="duel-score">{duel.score ?? "—"}</b></td></tr>)}</tbody>
            </table>
          </div>
