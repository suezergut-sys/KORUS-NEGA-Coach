...
describe("about product page", () => {
  ...
  it("contains every merged PR in newest-first order", () => {
    expect(PRODUCT_HISTORY).toHaveLength(130);
    expect(PRODUCT_HISTORY.map((item) => item.pr)).toEqual(
      [133, 132, 131, 130, 129, 128, 127, 126, 125, 124, 123, 122, 121, 120, 119, 118, 117, 116, 115, 114, 113, 112, 111, 110, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83, 82, 81, 80, 79, 78, 77, 76, 75, 74, 73, 72, 71, 70, 69, 68, 67, 66, 65, ...Array.from({ length: 61 }, (_, index) => 61 - index)],
    );
  ...
  it("groups versions by date without changing their order", () => {
    ...
    expect(groups[0].date).toBe("2026-08-29");
